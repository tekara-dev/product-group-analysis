(async () => {
  ...
  const treeSkeleton = document.getElementById("treeSkeleton");

  const analyzeSettings = document.getElementById("analyzeSettings");
  const analyzeSettingsSkeleton = document.getElementById(
    "analyzeSettingsSkeleton"
  );
  const sourceRussian = document.getElementById("sourceRussian");
  const sourceForeign = document.getElementById("sourceForeign");
  ...
  var currentPrice = "priceCustomToggle";
  var data = [];
  var errors = [];
  var sourceValues = {
    sourceRussian: false,
  ...
  addLoader("Загружаем настройки");

  const settings = await Promise.all([
    getServerData("getActivePriceType"),
    getServerData("getDocProps", ["analyzeSourceSettings"]),
  ]);

  currentPrice = settings[0] || "Цена поставщика";
  priceType.innerHTML = currentPrice;
  sourceValues = settings[1] || {
    sourceRussian: false,
    sourceForeign: false,
    sourceShadow: false,
  };

  removeLoader("Загружаем настройки");
  ...
  const syncAnalyzeSettings = () => {
    sourceRussianToggle.set(sourceValues.sourceRussian, false);
    sourceForeignToggle.set(sourceValues.sourceForeign, false);
    sourceShadowToggle.set(sourceValues.sourceShadow, false);
  };

  const handleAnalyzeSettingsChange = async (value, key) => {
    sourceValues[key] = value;
    addLoader("Сохраняем настройки");
    await getServerData("setDocProps", ["analyzeSourceSettings", sourceValues]);
    removeLoader("Сохраняем настройки");
  };
  sourceRussianToggle.on("change", (value) =>
    handleAnalyzeSettingsChange(value, "sourceRussian")
  );
  sourceForeignToggle.on("change", (value) =>
    handleAnalyzeSettingsChange(value, "sourceForeign")
  );
  sourceShadowToggle.on("change", (value) =>
    handleAnalyzeSettingsChange(value, "sourceShadow")
  );

  syncAnalyzeSettings();

  analyzeSettings.style.display = "";
  analyzeSettingsSkeleton.style.display = "";
  ...
  const refreshTree = async () => {
    ...
    treeSkeleton.style.display = "none";
    return { data, errors };
  };

  const _ = await refreshTree();

  analyzeSettingsSkeleton.style.display = "none";
